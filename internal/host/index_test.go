package host

import (
	"reflect"
	"testing"
)

func TestNewIndexSortsByGroupAndAlias(t *testing.T) {
	index, err := NewIndex([]Host{
		{Alias: "zulu", Group: "work"},
		{Alias: "beta", Group: "personal"},
		{Alias: "alpha", Group: "personal"},
	})
	if err != nil {
		t.Fatalf("NewIndex() error = %v", err)
	}
	got := aliases(index.All())
	if want := []string{"alpha", "beta", "zulu"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("aliases = %#v, want %#v", got, want)
	}
}

func TestNewIndexRejectsDuplicateAlias(t *testing.T) {
	_, err := NewIndex([]Host{{Alias: "Home"}, {Alias: "home"}})
	if err == nil {
		t.Fatal("NewIndex() error = nil, want duplicate error")
	}
}

func TestIndexOwnsAliasCopies(t *testing.T) {
	input := []Host{{Alias: "home", Aliases: []string{"home-alt"}}}
	index, err := NewIndex(input)
	if err != nil {
		t.Fatalf("NewIndex() error = %v", err)
	}
	input[0].Aliases[0] = "changed"
	result := index.All()
	result[0].Aliases[0] = "also-changed"
	if got := index.All()[0].Aliases[0]; got != "home-alt" {
		t.Fatalf("stored alias = %q, want home-alt", got)
	}
}

func TestIndexGetIsCaseInsensitive(t *testing.T) {
	index, err := NewIndex([]Host{{Alias: "Home", Aliases: []string{"home-alt"}}})
	if err != nil {
		t.Fatalf("NewIndex() error = %v", err)
	}
	got, ok := index.Get("home")
	if !ok || got.Alias != "Home" {
		t.Fatalf("Get() = %#v, %v", got, ok)
	}
	got.Aliases[0] = "changed"
	if stored, _ := index.Get("home"); stored.Aliases[0] != "home-alt" {
		t.Fatalf("Get() exposed internal slice: %#v", stored)
	}
	if got, ok := index.Get("HOME-ALT"); !ok || got.Alias != "Home" {
		t.Fatalf("Get() alternate alias = %#v, %v", got, ok)
	}
}

func TestNewIndexRejectsDuplicateAlternateAlias(t *testing.T) {
	_, err := NewIndex([]Host{{Alias: "home", Aliases: []string{"shared"}}, {Alias: "work", Aliases: []string{"shared"}}})
	if err == nil {
		t.Fatal("NewIndex() error = nil, want duplicate alternate alias")
	}
}

func aliases(hosts []Host) []string {
	result := make([]string, len(hosts))
	for i := range hosts {
		result[i] = hosts[i].Alias
	}
	return result
}
